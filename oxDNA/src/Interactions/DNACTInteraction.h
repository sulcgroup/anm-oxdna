...
#ifndef DNACT_INTERACTION_H_
#define DNACT_INTERACTION_H_
...
class DNACTInteraction: public DNA2Interaction<number> {
...
public:
	enum {
			...
			PRO_DNA_EXC_VOL = 10,
			PRO_ANG_POT = 11
			//Assigned 8 9 and 10 11 so it won't overwrite the already existing DNA function pointers in the _int_map
	};

    char _parameterfile[500];

	DNACTInteraction();
	virtual ~DNACTInteraction();
	...
	number _protein_ang_pot(BaseParticle<number> *p, BaseParticle<number> *q, LR_vector<number> *r, bool update_forces);
