...
template<typename number, typename number4>
CUDARNANMInteraction<number, number4>::~CUDARNANMInteraction() {
    //Delete All pointer arrays required for spring potential parameters
...
void CUDARNANMInteraction<number, number4>::get_settings(input_file &inp) {
	...
	this->RNANMInteraction<number>::get_settings(inp);
}
...
void CUDARNANMInteraction<number, number4>::cuda_init(number box_side, int N) {
    ...
    if (_read_par) {
        ...
        //allocate and initialize affected_len pointer array
        ...
        CUDA_SAFE_CALL(
                cudaMemcpy(_d_affected, _h_affected, 2 * spring_connection_num * sizeof(int), cudaMemcpyHostToDevice));

        CUDA_SAFE_CALL(cudaMalloc(&_d_aff_gamma, 2 * spring_connection_num * sizeof(int)));
        CUDA_SAFE_CALL(
                cudaMemcpy(_d_aff_gamma, _h_aff_gamma, 2 * spring_connection_num * sizeof(int), cudaMemcpyHostToDevice));

        CUDA_SAFE_CALL(cudaMalloc(&_d_aff_eqdist, 2 * spring_connection_num * sizeof(int)));
        CUDA_SAFE_CALL(
                cudaMemcpy(_d_aff_eqdist, _h_aff_eqdist, 2 * spring_connection_num * sizeof(int), cudaMemcpyHostToDevice));
        ...
        CUDA_SAFE_CALL(
                cudaMemcpy(_d_affected_indx, _h_affected_indx, (this->npro + 1) * sizeof(int), cudaMemcpyHostToDevice));
        ...
        float param_memory_mb = (spring_connection_num * 2 * sizeof(int) + 2 * spring_connection_num * 2 * sizeof(number)
                                 + (this->npro + 1) * sizeof(int)) / SQR(1024);
    ...
    //Constants for RNA/Protein Interactions (Same as DNA ones)
    ...
    if (_v_lists != NULL) {
        if (_v_lists->use_edge()) {
            ...
            rnanm_forces_edge_bonded<number, number4>
                    ...
                    (d_poss, d_orientations, d_forces, d_torques, d_bonds, this->_average, this->_use_mbf, this->_mbf_xmax, this->_mbf_finf, d_box, _d_aff_eqdist, _d_aff_gamma, _d_affected_indx, _d_affected);
        } else
